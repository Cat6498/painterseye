...
class PainterBase():
    ...
    def _normalize_strokes(self, v, weights):
        ...
        for y_id in range(self.m_grid):
            for x_id in range(self.m_grid):
                ...
                indi = sum(weights[:y_id * self.m_grid + x_id + 1])
                indj = sum(weights[:y_id * self.m_grid + x_id + 2])
                v[indi:indj, ys] = \
                    y_bias + v[indi:indj, ys] / self.m_grid
                v[indi:indj, xs] = \
                    x_bias + v[indi:indj, xs] / self.m_grid
                v[indi:indj, rs] /= self.m_grid

        return v


    def initialize_params(self, total_layer):

        self.x_ctt = np.random.rand(total_layer, self.rderr.d_shape).astype(np.float32)
        self.x_ctt = torch.tensor(self.x_ctt).to(device)

        self.x_color = np.random.rand(total_layer, self.rderr.d_color).astype(np.float32)
        self.x_color = torch.tensor(self.x_color).to(device)

        self.x_alpha = np.random.rand(total_layer, self.rderr.d_alpha).astype(np.float32)
    ...
    def stroke_sampler(self, weights):
        ...
        for i in range(self.m_grid*self.m_grid):
            if weights[i+1] > self.anchor_id:
              ...
              index = sum(weights[:i+1])+self.anchor_id
              
              self.x_ctt.data[index, :] = torch.tensor(
                  self.rderr.stroke_params[0:self.rderr.d_shape])
              self.x_color.data[index, :] = torch.tensor(
                  self.rderr.stroke_params[self.rderr.d_shape:self.rderr.d_shape+self.rderr.d_color])
              self.x_alpha.data[index, :] = torch.tensor(self.rderr.stroke_params[-1])
    ...
    def _forward_pass(self, weights):
        ...
        self.x = torch.vstack([self.x[np.sum(weights[:p]) : np.sum(weights[:p]) + self.anchor_id+1, :] if self.anchor_id < weights[p] else self.x[np.sum(weights[:p]) : np.sum(weights[:p])+ weights[p], :] for p in range(1, len(weights))])
        
        v = torch.reshape(self.x,
                          [self.x.shape[0], -1, 1, 1])
        ...
        for i in range(1,self.anchor_id+2):
          ...
          G_pred_foreground = torch.zeros([self.m_grid**2, 3, 128, 128], requires_grad=True).to(device)
          G_pred_foreground[fix] = self.G_pred_foregrounds[indexes]
          G_pred_alpha = torch.zeros([self.m_grid**2, 3, 128, 128], requires_grad=True).to(device)
          G_pred_alpha[fix] = self.G_pred_alphas[indexes]
          self.G_pred_canvas = G_pred_foreground * G_pred_alpha + self.G_pred_canvas * (1 - G_pred_alpha)
